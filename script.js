...
// ============================================================
const OR_API_KEY = "";
const OR_URL = OPENAI_URL;
const OR_VISION_MODEL = OPENAI_VISION_MODEL;
const OR_VISION_MODELS = [OPENAI_VISION_MODEL];
const GROQ_VISION_URL = OPENAI_URL;
const GROQ_VISION_MODEL = OPENAI_VISION_MODEL;
...
function startEdit(row, oldContent) {
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        await sendMessageToOpenAI(newText, row);
    });
}
...
Kamu diciptakan oleh Seorang Developer, dan kamu bangga menjadi ciptaannya.`;
    ...
    if (currentMode === 'smart') systemPrompt += `
You are in Smart AI mode. Provide deep analysis and clarity.`;
    else if (currentMode === 'thinking') systemPrompt += `
You are in Thinking AI mode. Give a clear, concise explanation of your reasoning without exposing private chain-of-thought.`;
    else if (currentMode === 'coding') systemPrompt += `
You are in Coding AI mode. Focus on programming solutions, algorithms, and clean code.`;
    else if (currentMode === 'fast') systemPrompt += `
You are in Fast AI mode. Respond quickly and concisely while still being accurate.`;

    if (!OPENAI_API_KEY || OPENAI_API_KEY === 'ISI_API_KEY_OPENAI_DI_SINI') {
        throw new Error('API key OpenAI belum diisi. Isi OPENAI_API_KEY terlebih dahulu.');
    }
    ...
    if (!result) throw new Error('OpenAI tidak mengembalikan jawaban.');
    return Array.isArray(result)
        ? result.map(part => typeof part === 'string' ? part : (part?.text || '')).join('').trim()
        : result;
}
...
async function callOpenRouterVision(
    ...
    if (!imageBase64) throw new Error('Data gambar kosong.');

    const allowedTypes = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
    if (!allowedTypes.includes(imageMimeType)) imageMimeType = 'image/jpeg';

    const style = customPromptStyle || 'normal';
    let systemPrompt = PROMPT_STYLES[style] || PROMPT_STYLES.normal;
    systemPrompt += `
Analisis gambar dengan teliti. Jika ada teks, baca sebisa mungkin. Jangan mengarang detail yang tidak terlihat.`;
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data?.error?.message || `OpenAI Vision HTTP ${res.status}`);
    let result = data?.choices?.[0]?.message?.content;
    if (Array.isArray(result)) result = result.map(part => typeof part === 'string' ? part : (part?.text || '')).join('').trim();
    if (!result) throw new Error('OpenAI Vision tidak mengembalikan jawaban.');
    return result.trim();
}
...
async function callOpenRouterVideoVision(videoFrames, userPrompt, metadata = {}) {
    ...
    let systemPrompt = PROMPT_STYLES[style] || PROMPT_STYLES.normal;
    systemPrompt += `
Video dikirim sebagai beberapa frame berurutan. Analisis urutan adegan dan jangan mengarang detail yang tidak terlihat.`;

    const content = [
        { type: 'text', text: userPrompt || 'Analisis video ini dengan detail.' }
    ];
    for (const frame of videoFrames) {
        const dataUrl = typeof frame === 'string'
            ? frame
            : (frame?.dataUrl || frame?.url || frame?.image || '');
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data?.error?.message || `OpenAI Video Vision HTTP ${res.status}`);
    let result = data?.choices?.[0]?.message?.content;
    if (Array.isArray(result)) result = result.map(part => typeof part === 'string' ? part : (part?.text || '')).join('').trim();
    if (!result) throw new Error('OpenAI Video Vision tidak mengembalikan jawaban.');
    return result.trim();
}
...
async function sendMessageToOpenAI(text, insertAfterRow = null) {
    if (isProcessing) return;
    welcomeState.style.display = 'none';
    isProcessing = true;
    sendBtn.disabled = true;
