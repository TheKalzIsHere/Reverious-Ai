...
if (hasVideo) {
    ...
    try {
        if (currentMode === 'thinking') {
            await showThinkingBubble('🎥 Mengambil beberapa cuplikan video untuk dianalisis...');
        }
        ...
        if (currentMode === 'thinking') {
            hideThinking();
            await showThinkingBubble(`🔍 Menganalisis ${sampled.frames.length} cuplikan dari video (${formatVideoDuration(sampled.duration)})...`);
        } else {
